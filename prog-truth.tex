...
\begin{definition}\label{Prog}
	For each $\xi<\kappa$ let $\wo_\xi(x)$ denote the formula 
	\[
	  \forall\gn {A(v)}\forall y\prec x\,\T_\xi(\gn{\TI(\dot y,A)}).
	\]
	For a formula $A(v)$ let $\Prog(A)$ be the formula
	\[ 
		\forall \alpha\,[\forall \zeta <\alpha\;A(\zeta) \,\to A(\alpha)].
	\]
\end{definition}
The first thing to notice about the formula $\wo_\xi(x)$ is that it is progressive.
%
\begin{lemma}\label{MR1}  
	We have $\HST_0^0\vdash \Prog(\wo_0)$.
\end{lemma} 

In essence using Gentzen's 1943 proof~\cite{Gentzen1943}, we get the following result.
\begin{lemma}\label{MR2} 
	$\HST_0^0\proves \forall \alpha'\forall \beta'\,[\wo_0(\alpha')\,\wedge\,\wo_0(\beta') \to \wo_0(\omega^{\alpha'}+\beta')]$.
\end{lemma}
...
(c) $\wo_0^1(\gamma)$ and (d) $\wo_0^1(\delta)$. Applying an inference \Nec{0}   \kommentar{$T_0$-Intro} to the formula in \cref{MR2} and taking (c) and (d) into account, we arrive at $\wo_0^1(\alpha)$.
...
via (U), and hence
$\T_0(\gn{\wo_0(\vartheta\dot{\rho})}$ holds.  By \Del{0}, $\wo_0(\vartheta\rho)$ follows.
\end{proof}
...
by \cref{MR2,MR1}.
%since the fact $\wo_0(\varepsilon_\alpha)$ is progressive in $\alpha$ is provable in $\HST_0^1$ (\emph{cf.}~\cref{classlem:S1e+}). 
To prove the latter,
...
\kommentar{
 and $\xi\in C_k(\sigma,\gamma_m)\cap\sigma$. Thus,
...
\Cref{prop:F0genwellordering} plays a key role in the analysis of $\HST_0^m$ and also $\HST_1^1$. \Cref{lem:F0tech} entails
\begin{align*}
