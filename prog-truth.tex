...
% version of the proof of \cref{lem:T8model} in the next section.}
...
%following rules: \emph{(Ax.1)}, $(\wedge)$, $(\lor)$, $(\omega)$ and $(\exists)$
...
%  \item [\normalfont(\Del\eta)]\label{inf:dag1} $\prov{\beta}{\rho}{\Gamma \sa 
...
%%	s_0)^\N$.
%  \item [\normalfont(\Rep\eta)]\label{inf:dag2} $\prov{\beta}{\rho}{\Gamma \sa 
...
Verification of the next proposition is straightforward and, hence, ommitted.
%Henceforth, we will omit reference to make free use of the weakening lemma them without mention.
\begin{proposition}\label{prop:T8prop1}\label{prop:T8items}\
 \begin{enumerate}
...
%   \label{lem:F8L1itemi}
	\item For every \( \L_\kappa \)-sentence \( A \), \( \prov{\omega}{0} {\lnot A,A} \)
  \item If \( A \) is a false equation, then\/ $\prov{\alpha}{\sigma}{\Gamma, A}$ implies
	$\prov{\alpha}{\sigma}{\Gamma}$.
	\label{lem:F8L1itemvi}
  \item If $s^{\N} \notin \setSent_\kappa$, then $\prov{\alpha}{\sigma}{\Gamma, \T_\eta s}$ implies $\prov{\alpha}\sigma{\Gamma}$.\label{lem:F8L1itemviji}\gelhighlight{We use assumptions about coding.}
  \item If\/ $\prov{\alpha}\sigma{\Gamma, A(s), A(t)}$ and
	$s^{\N}=t^{\N}$ then
	$\prov{\alpha}\sigma{\Gamma, A(s).}$\label{lem:F8L1itemx}
\end{enumerate}
\end{proposition}

For \( \alpha < \Omega \), \( \alpha^{\varepsilon+} \) denotes the least \( \varepsilon \)-number greater than \( \alpha \), i.e.,
\[
	\alpha^{\varepsilon+} = \sup \{ \alpha +1 , \omega^{\alpha +1}, \omega^{\omega^{\alpha+1}},\, \dotsc\, \}.
\]

The design of the calculus means that the cut rule is admissible with the same bounds as the arithmetical fragment.

\begin{theorem}[Admissibility of cut]\label{thm:T8CutElim}%(Cut admissibility for $\Tinfty$)
...
%The interpretation of \( \T_\eta \) in \( \mathfrak N_\rho \) is given by the
...
\begin{proposition}\label{prop:T8mono}
	If\/ $A$ is T-positive\gelnote{Defined T-positive?}, $\rho\le^*\sigma$ and $\mathfrak N_\rho\models A$ then $\mathfrak N_\sigma\models A$.
\end{proposition}
\begin{proof}
Suppose $\rho\le^*\sigma$. 
As $\rho^*<\vartheta\sigma$, we have
$\vartheta\rho\le\vartheta\sigma$ by \cref{prop:thetaitems1} (viii), which coupled with the fact that
$\rho|_\eta\le\sigma|_\eta$ implies that $\FF_{\rho, \eta } \subseteq \FF_{\sigma,\eta} $ for every $\eta<\kappa$. 
That $\mathfrak N_\rho\models A$ then $\mathfrak N_\sigma\models A$ for every T-positive sentence \( A \) follows by induction.
\end{proof}
...
\begin{lemma}\label{lem:T8Mprop}
 The following hold for every $\alpha<\Omega$, $\eta<\kappa$ and $\rho<\Omega^\kappa$.
 \begin{enumerate}
  \item $\mathfrak N_\rho\models\forall x\forall y( \T_\eta x \land	\T_\eta(x\dimplies y) \rightarrow \T_\eta y )$.\label{eqn:T8propii}
  \item $\mathfrak N_\rho\models\forall x(\T_\eta x \rightarrow \forall \xi<\bar\eta\,\T_\eta \gn{\T_\xi \dot x })$.\gelnote{More coding}\label{eqn:T8propiii}
  \item $\mathfrak N_\rho\models\forall\xi<\bar\kappa\forall x(\T_\eta \gn{\T_{\xi} \dot x } \rightarrow \T_\eta x )$ if, for every $\sigma<\rho$ and $\beta$ such that $ \max\{ \sigma^*,\beta \} <\vartheta\rho$,
	\begin{center}
	  $\prov\beta\sigma\Gamma$ implies $\mathfrak N_{\sigma+\beta}\models\textstyle\bigvee\Gamma$.
...
%  \item $\mathfrak N_\rho\models\forall\xi<\bar\kappa\forall \gn A(\T_\eta(\gn{\lnot \T_{\xi}(\gn A)})\rightarrow \T_\eta(\gn{\lnot A}))$.\label{eqn:T8propv}
  \item If $\rho|_\eta$ is not an $\Omega$-limit, $\mathfrak N_\rho\models
	\forall x\,\T_\eta \gn{A(\dot x)} $ and $\rho<^*\sigma$, then $\mathfrak N_{\sigma}\models \T_\eta \gn{\forall x\,A(x)}$.\label{eqn:T8propvi}
 \end{enumerate}
\end{lemma}
\begin{proof}
...
\ref{eqn:T8propvi}. Suppose $\mathfrak N_\rho\models \forall x\, \T_\eta \gn{A(\dot x)}$, that $\rho|_\eta$ is not an $\Omega$-limit. Let \( \rho' \) be such that $\rho <^* \rho' $. 
As \( \FF_{\rho,\eta} \) is non-empty, $\rho|_\eta > 0$ and there exist ordinals $\alpha_n$ and $\rho_n$ for each $n< \omega $ such that
\begin{gather*}
	\rho_n<\rho|_\eta,\\
	\max\{ \alpha_n,\rho_n^* \} <\vartheta\rho,\\
	\prov{\alpha_n}{\rho_n}{A(\bar n).}
\end{gather*}
As $\rho|_\eta$ is not an $\Omega$-limit there exists $\xi$ and $\tau$ such that $\rho|_\eta = \tau +\Omega^{\xi+1}$.
Let \( \sigma = \tau + \Omega^\xi \cdot \vartheta\rho \).
%By the Bounding lemma, for each \( n < \omega \) there exists \( \sigma_n \)
By the above, we have \( \rho_n <^* \sigma \) for every \( n \), so weakening implies that
\[
	\prov{\vartheta\rho}{\sigma}{ A(\bar n)} \text{ for every } n < \omega,
\]
whereby \( \prov{\vartheta\rho + 1}{\sigma}{ \forall x A(x)} \) follows.
Clearly, \( \sigma < \rho|_\eta \le \rho'|_\eta \) and \( \sigma^* \le \max\{ \rho^* , \vartheta\rho \} < \vartheta \rho' \).
Therefore, $\mathfrak N_{\rho'}\models \T_\eta \gn{\forall x\,A(x)}$ as desired.\gelhighlight{Reference to bounding lemma \cref{lem:T8conden} removed.}
\end{proof}
...
%  \item $\prov\alpha\rho{\lnot \T_\eta s}$ implies there is an $\L_\T$-sentence $A$ with $s^\N=\gn A$ such that $\prov\alpha\rho {\lnot A.}$
\end{enumerate}
\end{theorem}
...
arithmetical rules and axioms are dealt with easily. \Cref{prop:T8mono} deals
...
%It is easy to check that $\prov\alpha\rho{\lnot \T_\eta s}$ can only hold
...
% schema of $\HST_\beta$, we may deduce $\prov\alpha0A$ for some
% $\alpha<\varepsilon_0$, making use of the Cut Admissibility
% \cref{thm:T8CutElim}.
%
% For the axioms \textT\eta-* and remaining axioms of $\PA$, we may infer
% $\prov\omega{0}A$ by using the same arguments as in \cref{classprop:F8axF}
...
% \end{proof}
\begin{lemma}[Embedding \namecref{thm:T8Embed} for $\Tinfty$]\label{thm:T8Embed}
...
\omega)=\varphi \omega0$. \Cref{prop:T8embed} deals with the axioms of $\HST_0$. According to the
...
to consider, for which we employ \cref{lem:T8TElim}. $\lnot$\Conec0 is immediate, so suppose $\HST_0\proves \T_0(\gn
...
\Cref{prop:T8embed} deals with the axioms of $\HST_\beta$. According to the
...
\end{proof}
\Cref{thm:T8Embed} allows us to easily determine an upper bound on the strength of the theories $\HST_\beta$. By making use of the remarks at the beginning of this section, we see that the next theorem is provable with an almost identical argument to \cref{classthm:Fstrength}.
...
% TODO: Prove
% \begin{proof}
% All that remains from \cref{thm:T8Embed} is to show the results regarding
...
% that \cref{thm:T8Embed} can be rephrased in a similar manner to
...
% Formalising
% \cref{prop:T8items}, the cut admissibility theorem, \cref{thm:T8CutElim}
% and also the Bounding lemma, \cref{lem:T8conden}, is then standard ({\em
% cf.}~\cite[section 5]{Schw77}).
% \end{proof}
% \begin{corollary}\label{cor:Fmupperbd1}
...
% \end{corollary}
% We omit a proof of \cref{thm:FbetaupperbdI} because,
...
%
\begin{proof} 
\Cref{thm:FbetaupperbdI} provides both upper bounds for the two
theorems. 
...
% the T-Elimination theorem for $\Tinfty$, \cref{lem:T8TElim}.
...
\begin{thebibliography}{10}
	...
	\bibitem{Lei-thesis} \emph{Proof-theoretic investigations into the Friedman--Sheard theories and other theories of truth}, Ph.D.\@ thesis, School of Mathematics, University of Leeds, 2010.
\end{thebibliography}
