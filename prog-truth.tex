...
\begin{proposition}\label{prop:T8mono}
	If\/ $A$ is T-positive\gelnote{Defined T-positive?}, \( \rho \le^* \sigma \), $\rho|_\eta \le^*\sigma|_\eta $ for all \( \eta \) and $\mathfrak N_\rho\models A$, then $\mathfrak N_\sigma\models A$. \gelhighlight{Need stronger: \( \sigma|_\eta \le^* \rho|_\eta \) all \( \eta \).}
\end{proposition}
\begin{proof}
Suppose \( \rho \le^* \sigma \) and $\rho|_\eta \le^* \sigma|_\eta$ for all \( \eta \). 
As $\rho^*<\vartheta\sigma$, we have
$\vartheta\rho\le\vartheta\sigma$ by \cref{prop:thetaitems1}(\ref{itm:theta-comp}), which coupled with the fact that
\gelhighlight{$\rho|_\eta \le^* \sigma|_\eta$} implies that $\FF_{\rho, \eta } \subseteq \FF_{\sigma,\eta} $ for every $\eta<\kappa$. 
...
\begin{lemma}\label{lem:T8Mprop}
 ...
 \begin{enumerate}
  ...
  \item $\mathfrak N_\rho\models\forall\xi<\bar\kappa\forall x(\T_\eta \gn{\T_{\xi} \dot x } \rightarrow \T_\eta x )$ if, for every $\sigma <^* \rho$ and $\beta <\vartheta\rho$,
	\begin{center}
...
%  \item $\mathfrak N_\rho\models\forall\xi<\bar\kappa\forall \gn A(\T_\eta(\gn{\lnot \T_{\xi}(\gn A)})\rightarrow \T_\eta(\gn{\lnot A}))$.\label{eqn:T8propv}
  \item If $\rho|_\eta$ is not an $\Omega$-limit, $\mathfrak N_\rho\models
	\forall x\,\T_\eta \gn{A(\dot x)} $ and $\rho<^*\rho'$, then $\mathfrak N_{\rho'}\models \T_\eta \gn{\forall x\,A(x)}$.\label{eqn:T8propvi}
 \end{enumerate}
\end{lemma}
\begin{proof}
...
In particular, \( \sigma^* = \vartheta\rho \).
By the above, we therefore have \( \rho_n <^* \sigma \) for every \( n \), so weakening implies that
\[
	\prov{\vartheta\rho}{\sigma}{ A(\bar n)} \text{ for every } n < \omega.
\]
So \( \prov{\vartheta\rho + 1}{\sigma}{ \forall x A(x)} \).
Clearly, \( \sigma < \rho|_\eta \le \rho'|_\eta \) and \( \sigma^* = \vartheta\rho < \vartheta \rho' \).
Therefore, $\mathfrak N_{\rho'}\models \T_\eta \gn{\forall x\,A(x)}$.\\ \gelhighlight{Reference to bounding \cref{lem:T8conden} removed.}
\end{proof}
...
%\Cref{prop:T8mono} deals with (Weakening). 
